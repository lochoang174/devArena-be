...
export class CompileService {
    ...
    return new Observable<CompileStatus>((observer) => {
      (async () => {
        try {
          ...
          const expectedOutputs = await this.runSolutionCode(data.codeSolution, data.testcases,data.language);

          // 2. Chạy code của người dùng và so sánh với đáp án
          for (let i = 0; i < data.testcases.length; i++) {
            try {
              const process = await startCompilation(data.code, data.testcases[i].inputs,data.language);
  ...
  async runSolutionCode(codeSolution: string, testCases: TestCase[],language: string): Promise<string[]> {
    const solutionResults: string[] = [];
    for (let i = 0; i < testCases.length; i++) {
      try {
        const process = await startCompilation(codeSolution, testCases[i].inputs,language);
        ...
        try {
          ...
          for (let i = 0; i < data.testcases.length; i++) {
            try {
              ...
              // Run the compilation
              const compilationProcess = await startCompilation(data.code, data.testcases[i].inputs, data.language);
              const userOutput = await this.getProcessOutput(compilationProcess);
