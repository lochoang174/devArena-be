...
export class AuthController {
  ...
  async verifyOtp(@Body() verify: VerifyDTO) {
    const newUser = await this.authService.validateOtp(
      verify.email,
      verify.otpCode,
    );
    return { data: newUser };
  ...
  async update_password(
    ...
    return this.authService.updatePassword(user.id, updateDto);
  }

  @Get("/forgot-password/:email")
  @HttpCode(HttpStatus.NO_CONTENT)
  @Public()
  async forgot_password(@Param("email") email: string) {
  ...
  @Public()
  @HttpCode(HttpStatus.NO_CONTENT)
  async reset_password(@Body() reset: ResetPassworDto) {
